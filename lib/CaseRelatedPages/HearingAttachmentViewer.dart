...
class HearingAttachmentView extends StatefulWidget {
  final String attachmentId;
  final String jwtToken;

  const HearingAttachmentView({
    super.key,
  ...
  @override
  State<HearingAttachmentView> createState() => _HearingAttachmentViewState();
}

class _HearingAttachmentViewState extends State<HearingAttachmentView> {
  Uint8List? fileBytes;
  String? contentType;
  String? tempFilePath;
  String? webUrl;
  ...
  Future<void> loadAttachment() async {
    final baseUrl = BASE_URL.Urls().baseURL.replaceFirst('/api/', '/');

    final url = Uri.parse(
      '${baseUrl}hearing/attachment/view/${widget.attachmentId}',
    );
  ...
  Future<void> _openOrDownload() async {
    try {
      final url =
          "${BASE_URL.Urls().baseURL.replaceAll("/api/", "/")}hearing/attachment/${widget.attachmentId}";
