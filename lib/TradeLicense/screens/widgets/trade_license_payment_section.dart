// lib/TradeLicense/screens/widgets/trade_license_payment_section.dart
...
/// Fixed total fee for trade license registration
const double kTradeLicenseTotalFee = 5000.0;
const String kTradeLicenseReceiverPhone = '+8801874648472';
...
class TradeLicensePaymentSection extends StatefulWidget {
  ...
  @override
  State<TradeLicensePaymentSection> createState() =>
      _TradeLicensePaymentSectionState();
}

class _TradeLicensePaymentSectionState
    extends State<TradeLicensePaymentSection> {
  List<TradeLicensePaymentResponseDTO> _payments = [];
  ...
  Future<void> _loadPayments() async {
    ...
    final result = await TradeLicensePaymentService.findByTradeLicenseId(
        widget.tradeLicenseId);

    if (!mounted) return;

    if (result['status'] == 'success') {
      final list = TradeLicensePaymentService.parseList(result);
      setState(() {
        _payments = list;
        _isLoading = false;
      });
  ...
  double get _remaining {
    final r = kTradeLicenseTotalFee - _totalPaid;
    return r < 0 ? 0 : r;
  }
  ...
  double get _progress {
    if (kTradeLicenseTotalFee == 0) return 0;
    final p = _totalPaid / kTradeLicenseTotalFee;
    return p > 1 ? 1 : p;
  }
          ...
          Row(
            children: [
              const Icon(Icons.payments_outlined,
                  size: 20, color: Color(0xFF1E7A3A)),
    ...
    return Container(
      ...
      child: Column(
        children: [
          _summaryRow(
            'Total Fee',
            '৳ ${kTradeLicenseTotalFee.toStringAsFixed(0)}',
  ...
  Widget _paymentTile(TradeLicensePaymentResponseDTO p) {
