...
class DirectorService {
  ...
  Future<Director> addDirector({
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString("directorId", director!.id!);
...
// lib/DirectorsPages/director_service.dart
// Replace the entire updateDirector method

// lib/DirectorsPages/director_service.dart
// Replace the updateDirector method with this:
...
Future<Director> updateDirector({
  ...
  if (response.statusCode == 200) {
    // ✅ Update succeeded, now return a Director object
    // If the response has data, parse it; otherwise use the data we have
    ...
    if (responseBody.body.isNotEmpty && responseBody.body.trim() != '') {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString("directorId", updatedDirector!.id!);
          return updatedDirector;
        }
      } catch (e) {
        print('⚠️ Error parsing response: $e');
        // Fall through to return the director we have
      }
    }
    
    // ✅ If we reach here, either the response was empty or parsing failed
    // So we return a Director object with the data we have
    print('✅ Update successful, returning director with ID: $id');
    
    // Save the directorId to SharedPreferences
    final prefs = await SharedPreferences.getInstance();
  ...
  Future<List<DirectorResponse>> getAllDirectors() async {
    ...
    final uri = Uri.parse('${baseUrl}directors');
  ...
  Future<DirectorResponse> getDirectorByUserId(String? userId) async {
    await _getToken();
